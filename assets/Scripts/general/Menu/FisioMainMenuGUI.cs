...
public class FisioMainMenuGUI : MonoBehaviour {
	...
	bool fisio = true;
	bool player, menu, login, signup, fisioLogError, playerLogError, signError, passwError;
	bool exists = false;
	...
	void FisioWindow (int id){
		...
		if (GUI.Button(new Rect((windowRect.width - 210)/2, 240, 210, 75), "Esci")){
			Application.Quit();
		}
	}
